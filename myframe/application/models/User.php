...
class User extends Model {
	...
	public	function clear(){
		$this->ndb->delete("delete from cmstop_admin where `userid`>9");
		$this->ndb->delete("delete from cmstop_member where `userid`>9");
		$this->ndb->delete("delete from cmstop_member_detail where `userid`>9");
		self::auto_increment('cmstop_member',10);
		self::auto_increment('cmstop_member_detail',10);
		self::auto_increment('cmstop_admin',10);
		self::log('删除表：cmstop_member的主键userid>9的用户数据。设置主键从10开始自增！');
		self::log('删除表：cmstop_member_detail的主键userid>9的用户数据。设置主键从10开始自增！');
	}
	...
	public function insertnewdb($data){
		...
		$option['member']['groupid'] = $data['groupid'] ? $data['groupid'] :6;
