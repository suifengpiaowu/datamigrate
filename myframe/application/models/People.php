...
class People extends Model {
	...
	public	function runsynthetical(){
		...
		//获取数据总条数
		$total = $this->odb->page("select count(a.id) total from reform_people a")[0]['total'];

		//获取数据总页数
		$pages = ceil($total/$this->length); 
		echo "ALL-line $total ALL pages $pages </br>"."\r\n";
		$field = "a.id,a.title,a.thumb,a.keywords,a.url,a.inputtime,a.updatetime,a.description,a.content,a.banner,a.typeids,a.index_template,a.specialid,a.peopleid,a.initials";
		for ($i=1; $i <= $pages; $i++) { 
			$sql = "select $field from reform_people a order by a.id asc";
			// var_dump($sql);
	...
	function insertsynthetical($contentid,$data){
		...
		$option['synthetical']['kindid'] = 2; //数据类型1图书与,2人物,3机构,4事件
		...
		$option['synthetical']['initials'] = $data['initials'] ? substr($data['initials'],0,255) : null;
		...
		if($res){
			...
			if($data['typeids']) self::insertproperty($contentid,$data['typeids']);
	...
	function insertproperty($contentid,$typeids){
		if($typeids == 51){
			$proid = 76;
		}elseif($typeids == 52){
			$proid = 77;
	...
	function insertlog($contentid,$data){
		$idata = array(
										...
										'oldid'=>$data['id'],
										'oldurl'=>$data['url'],
										'message'=>'人物数据迁移，newid为新站contentid，oldid为旧站id',
										);
	...
	public	function clear(){
		...
		$this->ndb->delete("delete from cmstop_synthetical where `kindid`=2");
		...
		$this->ndb->delete("delete from $this->_table where `catid`=6");
		$this->datadeal->auto_increment($this->_table);
		write_file("cmstop_content","删除表：$this->_table的catid=4的图书数据。设置主键自增！");

		$this->ndb->delete("delete from cmstop_removal_log where `type`='People'");
